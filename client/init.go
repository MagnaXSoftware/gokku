...
var initCmd = func() *initCommand {
	...
	cmd.FlagSet.StringVarP(&CurrentConfig.Username, "user", "u", "gokku", "Username used for the remote ssh connection, often gokku or dokku.")
	cmd.FlagSet.StringVarP(&CurrentConfig.Hostname, "hostname", "h", "", "The hostname of the gokku/dokku server.")
	cmd.FlagSet.IntVarP(&CurrentConfig.Port, "port", "p", 22, "The port on which the gokku/dokku binary can be reached (over ssh).")
	cmd.FlagSet.StringVarP(&CurrentConfig.KeyFile, "key", "k", "", "The path to the ssh key to use for the connection.")
...
func (cmd *initCommand) Execute(args []string) int {
	...
	data, err := yaml.Marshal(struct{ Gokku *ClientConfig }{&CurrentConfig})
